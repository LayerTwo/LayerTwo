...
export class l2_local_leaflet_map_list_problems extends l2_local_leaflet_map {
    ...
    leaflet_map_init() {
        ...
        document.addEventListener("l2-local-problem-list-marker-clicked", this.local_problem_selected.bind(this), true);
    ...
    create_marker(data) {
        ...
        this.marker_array.push(L.marker([parseFloat(data.detail.local_problem_latitude), 
                                         parseFloat(data.detail.local_problem_longitude)], 
                                         { icon: this.marker_normal_icon, 
                                           uuid: data.detail.local_problem_uuid }).on('mouseover', function (e) {
                                                                                  this.openPopup();})
                                                                                  .on('mouseout', function (e) {
                                                                                  this.closePopup();})
                                                                                  .on('click', event => this.dispatchEvent(new CustomEvent("l2-local-problem-list-marker-clicked", { detail:{local_problem_uuid: data.detail.local_problem_uuid}, bubble: true, composed: true })))
                                                                                  .bindPopup(data.detail.local_problem_title, {autoClose: false}).addTo(this.leaflet_map));
