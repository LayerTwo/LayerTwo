...
export class l2_leaflet_map extends HTMLElement {
	...
	get_credentials() {
		if (document.getElementsByName("leaflet_app_id")[0] && document.getElementsByName("leaflet_app_code")[0]) {
			this.leaflet_app_id = document.getElementsByName("leaflet_app_id")[0].getAttribute("content");
			this.leaflet_app_code = document.getElementsByName("leaflet_app_code")[0].getAttribute("content");
		}
	}
	...
	construct_map() {
		...
		L.tileLayer.provider('OpenStreetMap.Mapnik', { app_id: this.leaflet_app_id, app_code: this.leaflet_app_code }).addTo(this.leaflet_map);
	...
	select_marker(event) {
		this.marker_array.forEach(function (item, index, array) {
			if (item.options.uuid === event.detail.element_uuid && this.sub_section !== "Projects") {
