export class l2_start_project_form extends HTMLElement {
    ...
    populate_project_data(event) {
        ...
        this.handle_add_solution_clicked();

    }

    create_associated_element(event) {
        this.associated_element_uuid = event.detail.element_uuid;
    ...
    create_solution_element() {
        ...
        this.solutions_list.appendChild(solution_form);
        this.add_step(element_id);
    }
    ...
    add_step(element_id) {
        ...
        step_description_heading.innerHTML = "Description: ";
    ...
    submit() {
        this.solutions_and_steps_package = new Object();
        this.solution_elements_list.forEach(solution_element => {
            this.solution_steps_list.forEach(element_step => {
                if (solution_element in element_step){
                    this.solutions_and_steps_package[`${solution_element}`] = new Object();
                    this.solutions_and_steps_package[`${solution_element}`]["Solution"] = new Object();
                    this.solutions_and_steps_package[`${solution_element}`]["Solution"]["Title"] = this.querySelector(`#solution-title-input-${solution_element}`).value;
                let i = 1;
                element_step[solution_element].forEach(step => {
                    this.solutions_and_steps_package[`${solution_element}`][i] = new Object();
                    this.solutions_and_steps_package[`${solution_element}`][i]["Title"] = this.querySelector(`#step-title-id-${step}`).value;
                    this.solutions_and_steps_package[`${solution_element}`][i]["Description"] = this.querySelector(`#step-description-id-${step}`).value;
                    console.log(this.solutions_and_steps_package);
                    console.log(this.associated_element_uuid);
                    i++;
                });
            }
            });
        ...
        this.dispatchEvent(new CustomEvent("ws-channel-mbox", {
            detail: {
                ...
                solutions_package: this.solutions_and_steps_package,
                associated_element:this.associated_element_uuid,
    ...
    html_solution_title(element_id) {
        return `<div>Solution: <input id="solution-title-input-${element_id}" form="project-submit-form" class="solution-title-input" type="text" required></div>
        `;
    }
