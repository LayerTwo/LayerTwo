...
export class l2_local_edit_problem extends HTMLElement {
    constructor() {
        super();
        ...
        document.addEventListener("l2-local-problems-edit-local-problem", this.handle_problem_update.bind(this), true);
        document.addEventListener("l2-city-local-problem-update-success", this.handle_update_success.bind(this), true);
        document.addEventListener("l2-city-local-problem-update-failed", this.handle_update_failure.bind(this), true);
    }
    ...
    render_template() {
        ...
        this.shadowRoot.querySelector("#local-problems-form").addEventListener("submit", event => this.handle_update_form(event));
        this.shadowRoot.querySelector("#form-gps-button").addEventListener("pointerdown", event => this.get_gps_coordinates_from_device());
        this.shadowRoot.querySelector("#cancel-update-button").addEventListener("pointerdown", event => this.handle_cancel_update());
        this.shadowRoot.querySelector("#form-latitude-input").addEventListener("input", event => this.handle_input_gps_coords_change());
        this.shadowRoot.querySelector("#form-longitude-input").addEventListener("input", event => this.handle_input_gps_coords_change());
    }
    ...
    handle_input_gps_coords_change() {
        this.manual_latitude = parseFloat(this.shadowRoot.querySelector("#form-latitude-input").value);
        this.manual_longitude = parseFloat(this.shadowRoot.querySelector("#form-longitude-input").value);
        this.dispatchEvent(new CustomEvent("l2-update-local-problem-form-map-marker-position", { detail: { position: { coords: { latitude: this.manual_latitude, longitude: this.manual_longitude } } }, bubble: true, composed: true }));
    }
    ...
    handle_cancel_update() {
        this.form_controls_enable();
        this.dispatchEvent(new CustomEvent("l2-local-problems-edit-local-problem-canceled", { bubble: true, composed: true }));
    }

    form_controls_enable() {
        ...
        this.problems_form.querySelector("#form-problem-title-input").disabled = false;
        ...
        this.problems_form.querySelector("#update-button").disabled = false;
        this.problems_form.querySelector("#cancel-update-button").disabled = false;
    }

    handle_update_form(event) {
        event.preventDefault();
        this.submit_form_container = this.shadowRoot.querySelector("#input-form-container");
        this.problems_form = this.submit_form_container.querySelector("#local-problems-form");
        this.problems_form.querySelector("#form-problem-title-input").disabled = true;
        ...
        this.problems_form.querySelector("#update-button").disabled = true;
        this.problems_form.querySelector("#cancel-update-button").disabled = true;
    ...
    handle_update_success() {
        ...
        this.problems_form.querySelector("#form-problem-title-input").value = "";
        ...
        this.submit_success_msg_element.innerHTML = `Update Accepted`;
        if (this.entry_result === undefined) {
            this.submit_form_container.querySelector("#update-button-container").querySelector("#submit-result-container").appendChild(this.submit_success_msg_element);
            this.entry_result = "accepted";
        } else if (this.entry_result === "failed") {
            this.submit_form_container.querySelector("#update-button-container").querySelector("#submit-result-container").querySelector("#submit-failed-text").remove();
            this.submit_form_container.querySelector("#update-button-container").querySelector("#submit-result-container").appendChild(this.submit_success_msg_element);
            this.entry_result = "accepted";
        }
        this.dispatchEvent(new CustomEvent("l2-local-problems-edit-local-problem-complete", { detail:{local_problem_uuid: this.local_problem_uuid}, bubble: true, composed: true }));
        this.submit_success_msg_element.innerHTML = ``;
        this.local_problem_uuid = "";
    }
    ...
    handle_update_failure() {
        ...
        this.submit_failed_msg_element.innerHTML = `Update Rejected`;
        if (this.entry_result === undefined) {
            this.submit_form_container.querySelector("#update-button-container").querySelector("#submit-result-container").appendChild(this.submit_failed_msg_element);
            this.entry_result = "failed";
        } else if (this.entry_result === "accepted") {
            this.submit_form_container.querySelector("#update-button-container").querySelector("#submit-result-container").querySelector("#submit-success-text").remove();
            this.submit_form_container.querySelector("#update-button-container").querySelector("#submit-result-container").appendChild(this.submit_failed_msg_element);
            this.entry_result = "failed";
        }
    }
    ...
    submit_form(event) {
        this.form_problem_title = this.shadowRoot.querySelector("#form-problem-title-input").value;
        ...
        this.dispatchEvent(new CustomEvent("l2-city-local-problem-update-form-data-submit", {
            detail: {
                problem_uuid: this.local_problem_uuid,
                problem_title: this.form_problem_title,
    ...
    handle_problem_update(data) {
        this.form_controls_enable();
        this.local_problem_uuid = data.detail.local_problem_uuid;
        this.problems_form.querySelector("#form-latitude-input").value = data.detail.local_problem_latitude;
        this.problems_form.querySelector("#form-longitude-input").value = data.detail.local_problem_longitude;
        this.handle_input_gps_coords_change();
        this.problems_form.querySelector("#form-problem-title-input").value = data.detail.local_problem_title;
        this.problems_form.querySelector("#form-problem-importance-slider").value = data.detail.local_problem_importance;
        this.problems_form.querySelector("#form-description-input").value = data.detail.local_problem_description;
    }
    ...
    template() {
        ...
        <div id="input-form-container">
          ...
          <div id="form-problem-heading">Edit Problem Form</div>

          <div id="form-problem-title">Title:</div>
          <input id="form-problem-title-input" required type="text">
          ...
          <input id="update-button" value="Update" type="submit"/>
          <input id="cancel-update-button" value="Cancel" type="button"/>
...
customElements.define("l2-local-edit-problem", l2_local_edit_problem)
