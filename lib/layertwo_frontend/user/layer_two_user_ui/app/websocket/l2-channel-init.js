export class l2_channel_init extends HTMLElement {
    ...
    update_basic_info(basic_info_db_data) {
        if (basic_info_db_data.error === "false") {
            ...
            sessionStorage.setItem("city_local_default_zoom", basic_info_db_data.city_local_default_zoom);
            sessionStorage.setItem("city_default_zoom", basic_info_db_data.city_default_zoom);
            sessionStorage.setItem("country_default_zoom", basic_info_db_data.country_default_zoom);
            this.dispatchEvent(new CustomEvent("l2-update-entity-basic-info", { bubble: true, composed: true }));
            this.l2_init_join_all_channels();
        } else {
